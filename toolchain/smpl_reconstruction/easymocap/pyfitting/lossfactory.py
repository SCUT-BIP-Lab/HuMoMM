...
class LossRegPosesZero:
    def __init__(self, keypoints, cfg) -> None:
        ...
        if keypoints.shape[-2] <= 15:
            use_feet = False
            use_head = False
        elif keypoints.shape[-2]==21: # body21
            use_feet = False
            use_head = False
        elif keypoints.shape[-2]==24: # smpl 24 joints
            use_feet = False
            use_head = False
        else:
...
class LossKeypointsMV2DCam(LossRepro):
    def __init__(self, keypoints2d, bboxes, cam_params, cfg) -> None:
        """
        Args:
            keypoints2d (ndarray): (nViews, nFrames, nJoints, 3)
            bboxes (ndarray): (nViews, nFrames, 5)
    ...
    def __call__(self, kpts_est, **kwargs):
        ...
        for i in range(self.nViews):
            Ks.append(kwargs[f'K_{i}'])
            Rvecs.append(kwargs[f'Rvec_{i}'])
            Ts.append(kwargs[f'T_{i}'])
        ...
        # Pall=[torch.matmul(K, RT) for K, RT in zip(Ks, new_RTs)]

        Pall=[torch.matmul(K, RT) for K, RT in zip(Ks, RTs)]
        Pall=torch.stack(Pall, dim=0)
        kpts_homo = torch.cat([kpts_est[..., :self.nJoints, :], self.kpt_homo], dim=2)
        point_cam = torch.einsum('vab,fnb->vfna', Pall, kpts_homo)
        img_points = point_cam[..., :2]/point_cam[..., 2:]
        return super().__call__(img_points)/self.nViews/self.nFrames
