...
from .lossfactory import LossKeypointsMV2DCam, LossSmoothBodyMean, LossRegPoses
...
def optimizeShape(body_model, body_params, keypoints3d,
    ...
    def closure(debug=False):
        optimizer.zero_grad()
        keypoints3d = body_model(return_verts=False, return_tensor=True, return_smpl_joints=True, only_shape=True, **body_params)
...
def _optimizeSMPL(body_model, body_params, prepare_funcs, postprocess_funcs, 
    ...
    def closure(debug=False):
        ...
        kpts_est = body_model(return_verts=False, return_tensor=True, return_smpl_joints=True, **new_params)
...
def _optimizeSMPLCam(body_model, body_params, prepare_funcs, postprocess_funcs, 
    loss_funcs, extra_params=None,
    weight_loss={}, cfg=None):
    """ A common interface for different optimization.
    ...
    if cfg.verbose:
        ...
        for key, func in loss_funcs.items():
            print('  -> {:15s}: {}'.format(key, func.__doc__))
    extra_cam_params=extra_params['Ks']+extra_params['Rvecs']+extra_params['Ts']
    opt_params = get_optParams(body_params, cfg, extra_cam_params)
    grad_require(opt_params, True)
    optimizer = LBFGS(opt_params, 
        line_search_fn='strong_wolfe')
    PRINT_STEP = 100
    ...
    def closure(debug=False):
        ...
        for i, K in enumerate(extra_params['Ks']):
            new_params[f'K_{i}']=K
        for i, Rvec in enumerate(extra_params['Rvecs']):
            new_params[f'Rvec_{i}']=Rvec
        for i, T in enumerate(extra_params['Ts']):
            new_params[f'T_{i}']=T
...
def optimizePose2DCam(body_model, params, bboxes, keypoints2d, Pall, weight, cfg, dataset):
    """ 
        simple function for optimizing model pose given 3d keypoints
    ...
    device=cfg.device
    Ks, Rvecs, Ts =[], [], []
    for cam_id in dataset.cams: # 
        cam=dataset.cameras[cam_id]
        Ks.append(torch.tensor(cam['K'], dtype=torch.float32).to(device))
        Rvecs.append(torch.tensor(cam['Rvec'], dtype=torch.float32).to(device))
        Ts.append(torch.tensor(cam['T'], dtype=torch.float32).to(device))
    cam_params={'Ks': Ks, 'Rvecs': Rvecs, 'Ts': Ts}
    ...
    loss_funcs = {
        'k2d': LossKeypointsMV2DCam(keypoints2d, bboxes, cam_params, cfg).__call__,
        'smooth_body': LossSmoothBodyMean(cfg).body,
        'init_poses': LossInit(params, cfg).init_poses,
        'smooth_poses': LossSmoothPoses(nViews, nFrames, cfg).poses,
        'reg_poses': LossRegPoses(cfg).reg_body,
    ...
    # cfg.verbose=True
    params = _optimizeSMPLCam(body_model, params, prepare_funcs, postprocess_funcs, loss_funcs, weight_loss=weight, cfg=cfg, extra_params=cam_params)
    update_cam_params(dataset.cameras, cam_params)
    return params



def update_cam_params(cams, cam_params):
    import cv2
    for i, cam in enumerate(cams.keys()):
        K=cam_params['Ks'][i].detach().cpu().numpy()
        ...
        Rvec=cam_params['Rvecs'][i]
        T=cam_params['Ts'][i]

        R=angle_axis_to_rotation_matrix(Rvec.transpose(0, 1))[0]
        RT=torch.hstack((R, T))
        ...
        # R=RT[:3, :3]
        # T=RT[:,3][:, None]
        # Rvec=rotation_matrix_to_angle_axis(R[None,...].contiguous())
        # new_R=angle_axis_to_rotation_matrix(Rvec)[0]
        # new_RT=torch.hstack((new_R, T))
        # RT=new_RT.detach().cpu().numpy()
