...
def trans_smpl_with_cam(smpl_annots, cam_param):
    ...
    for i, smpl_annot in enumerate(smpl_annots):
        ...
        # R_rot=angle_axis_to_rotation_matrix(torch.tensor(np.array(Rh)[None, :]))
        # trans_R_rot=torch.bmm(torch.tensor(R[None,...]), R_rot)
        # trans_Rh=rotation_matrix_to_angle_axis(trans_R_rot)[0].detach().cpu().numpy()
        # trans_Rh=trans_Rh[:, None]
...
def main(args):
    ...
    for video_path in video_pathes:
        ...
        # process 2d label
        process_2d_labels(args.tmp_dir, skel_type='smpl')
        ...
        # generate smpl label
        exe_command(
            f"python apps/demo/mv1p.py {args.tmp_dir} --out {args.tmp_dir}/output --body smpl --opt_cam")

        # trans smpl annot with cam and save to output dir
        src_smpl_files = sorted(glob.glob(osp.join(args.tmp_dir, 'output', 'smpl', '*.json')))

        if osp.exists(os.path.join(args.tmp_dir, 'intri_opt.yml')):
            opt_cam_params = read_camera(os.path.join(args.tmp_dir, 'intri_opt.yml'),
                                         os.path.join(args.tmp_dir, 'extri_opt.yml'))
        else:
            opt_cam_params = read_camera(os.path.join(args.tmp_dir, 'intri.yml'),
                                         os.path.join(args.tmp_dir, 'extri.yml'))

        for frame_idx, smpl_file in enumerate(src_smpl_files):
            smpl_annot = read_json(smpl_file)
            for cam_name in opt_cam_params['basenames']:
                trans_smpl_annot = trans_smpl_with_cam(smpl_annot, opt_cam_params[cam_name])
                smpl_path = osp.join(args.output_root, cam_name, f'{cam_name}RF{frame_idx:03d}.json')
                save_json(smpl_path, trans_smpl_annot)
        # copy opt cam params to dst dir
        if osp.exists(os.path.join(args.tmp_dir, 'intri_opt.yml')):
            for cam_name in opt_cam_params['basenames']:
                dst_intri_path = osp.join(osp.dirname(args.output_root), 'Opt_cam', cam_name, 'intri_opt.yml')
                dst_extri_path = osp.join(osp.dirname(args.output_root), 'Opt_cam', cam_name, 'extri_opt.yml')
                if not osp.isdir(osp.dirname(dst_intri_path)):
                    os.makedirs(osp.dirname(dst_intri_path))
                if not osp.isdir(osp.dirname(dst_extri_path)):
                    os.makedirs(osp.dirname(dst_extri_path))
                shutil.copy2(os.path.join(args.tmp_dir, 'intri_opt.yml'), dst_intri_path)
                shutil.copy2(os.path.join(args.tmp_dir, 'extri_opt.yml'), dst_extri_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_root', type=str, default='../../dataset/scut_key_frame/Smpl',
