...
def trans_2d_joints(joints_annots):
    results={}
    results['shapes']=[]
    for i,  annots in enumerate(joints_annots['annots']):
        person_id=annots['personID']
        joints_2d=annots['keypoints']
        if len(joints_2d)!=21:
            print('num joints less than 21, exit!')
            exit()
        for joint_idx, joint in enumerate(joints_2d):
            if joint[-1]==0.: # unvisble
                trans_joints=[[0., 0.],]
                results['shapes'].append({
                    'label': f'{joint_idx:02d}',
...
def main(args):
    ...
    for video_path in video_pathes:
        ...
        for video_dir in mv_video_pathes:
            ...
            shutil.copytree(video_dir.replace('RGB_frame', 'Label_det'), os.path.join(
        ...
        # save repro 2d joint
        exe_command(
            f"python apps/demo/save_track_repro.py {args.tmp_dir} --out {args.tmp_dir}/output-track --body body21")

        for cam_name in cam_params['basenames']:
            cam_dir=os.path.join(args.tmp_dir, 'output-track', 'keypoints2d', cam_name)
            json_files=os.listdir(cam_dir)
            for json_file in json_files:
                src_json_path=os.path.join(cam_dir, json_file)
                joints_annots=read_json(src_json_path)
                trans_joints_annots=trans_2d_joints(joints_annots)
                img_name=json_file.replace('json', 'jpg')
                trans_joints_annots['imagePath']=img_name
                img_path=os.path.join(args.tmp_dir, 'images', cam_name, img_name)
                img=cv2.imread(img_path)
                trans_joints_annots['imageData']=img_arr_to_b64(cv2.cvtColor(img, cv2.COLOR_BGR2RGB)).decode('utf-8')
                joints_path = osp.join(args.output_2d_joint_root, cam_name, json_file)
                save_json(joints_path, trans_joints_annots)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_2d_joint_root', type=str, default='../../dataset/scut_key_frame/Label_3d',
