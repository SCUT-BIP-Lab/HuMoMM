...
from easymocap.mytools.camera_utils import write_extri, write_intri
...
def mv1pmf_smpl(dataset, args, weight_pose=None, weight_shape=None):
    ...
    params = smpl_from_keypoints3d2d(body_model, kp3ds, keypoints2d, bboxes, 
        ...
        weight_shape=weight_shape, weight_pose=weight_pose, dataset=dataset)
    ...
    for nf in tqdm(range(start, end), desc='render'):
        ...
        if args.vis_repro:
            keypoints = body_model(return_verts=False, return_smpl_joints=True, return_tensor=False, **param)[0] # use smpl24 style, so return_smpl_joints=True
...
""".format(args.path, ', '.join(args.sub), args.out, 
    ...
    if args.opt_cam:
        write_intri(os.path.join(args.path, 'intri_opt.yml'), dataset.cameras)
        write_extri(os.path.join(args.path, 'extri_opt.yml'), dataset.cameras)
