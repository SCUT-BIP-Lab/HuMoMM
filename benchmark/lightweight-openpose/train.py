...
from utils.load_state import load_state, load_from_mobilenet
...
def train(prepared_train_labels, train_images_folder, num_refinement_stages, base_lr, batch_size, batches_per_iter,
          ...
          val_labels, val_images_folder, val_output_name, checkpoint_after, val_after, prune, arch, input_height, dataset_name):
    # set some configs
    if arch == 'mobilenetv2':
        stride, upsample_ratio = 8, 4
        sigma, path_thickness = 7, 1
    elif 'hrnet' in arch:
        num_refinement_stages = 0  # no refinement stage for hrnet backbone
        stride, upsample_ratio = 4, 2
        sigma, path_thickness = 8, 1.5
    else:
        raise ValueError('The arch is incorrect, please check!')

    dataset = get_train_dataset(dataset_name, labels=prepared_train_labels, images_folder=train_images_folder,
                                stride=stride, sigma=sigma, paf_thickness=path_thickness, input_size=input_height)
    train_loader = DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)

    val_dataset = get_val_dataset(dataset_name, labels=val_labels, images_folder=val_images_folder,)

    net = get_pose_estimation_model(arch, num_heatmaps=dataset.num_joints + 1, num_pafs=dataset.num_pafs)

    optimizer = get_optimizer(arch, net, base_lr)
    ...
    if checkpoint_path:
        ...
        if from_mobilenet:
            ...
            checkpoint = {'state_dict': checkpoint} if 'state_dict' not in checkpoint.keys() else checkpoint
            load_state(net, checkpoint)
            if prune:
                from utils.nni_pruning import prune_light_openpose_model
                prune_light_openpose_model(net)
            if not weights_only:
    ...
    net.train()
    total_losses = [0, 0] * (num_refinement_stages + 1)  # heatmaps loss, paf loss in every log stage
    for epochId in range(current_epoch, 280):
        ...
        for batch_data in train_loader:
            ...
            if num_iter % log_after == 0:
                print('Epoch: {}, Iter: {}'.format(epochId, num_iter))
                for loss_idx in range(len(total_losses) // 2):
                ...
                save_batch_heatmaps(images, keypoint_maps, './tmp/heatmaps.png')
            ...
            if num_iter % checkpoint_after == 0:
                ...
                torch.save({'state_dict': net.module.state_dict(),
                           ...
                           snapshot_name)
                if prune:
                    snapshot_name = '{}/checkpoint_iter_{}_pruned.pth'.format(checkpoints_folder, num_iter)
                    torch.save(net.module, snapshot_name)

            if num_iter % val_after == 0:
                print('Validation...')
                evaluate(val_dataset, net, val_output_name, input_height, stride, upsample_ratio)
                net.train()
        time.sleep(3)  # sleep to avoid deadlock in dataloader
        epoch_duration = time.time() - start_time
        print(f'epoch time: {int(epoch_duration // 3600):d} Hours, {int(epoch_duration % 3600 // 60):d} Minutes, {int(epoch_duration % 3600 % 60):d} Seconds.')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--val-output-name', type=str, default='./results/detections.json',
    ...
    parser.add_argument('--input-size', type=int, default=256, help='image size to input the network')
    args = parser.parse_args()

    checkpoints_folder = './logs/{}/checkpoints'.format(args.experiment_name)
    ...
    train(args.prepared_train_labels, args.train_images_folder, args.num_refinement_stages, args.base_lr, args.batch_size,
          ...
          args.checkpoint_after, args.val_after, args.prune, args.arch, args.input_size, args.dataset)
