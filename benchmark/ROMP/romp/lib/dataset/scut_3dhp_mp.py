...
def SCUT_3DHP_MP(base_class=default_mode):
    class SCUT_3DHP_MP(Base_Classes[base_class]):
        def __init__(self, train_flag=True, split='train', **kwargs):
            super(SCUT_3DHP_MP, self).__init__(train_flag, regress_smpl=True)
            self.data_folder = os.path.join(self.data_folder, 'scut2_mp')
            ...
            # split train validation and test
            # train_action_id=[0, 1, 2, 3, 4, ]
            # val_action_id=[5, 6, 7, 8, 9]
            # test_action_id=[5, 6, 7, 8, 9]
            # if split=='train':
            ...
            logging.info('Loaded SCUT-3DHP {} set,total {} samples'.format('test', self.__len__()))
        ...
        def get_image_info(self, index):
            ...
            for i, (kp2d, kp3d) in enumerate(zip(self.annots[imgpath]['kp2d'], self.annots[imgpath]['kp3d'])):
                kp2ds.append(self.map_kps(kp2d,maps=self.kp2d_mapper))
                kp3ds.append(self.map_kps(kp3d,maps=self.kp3d_mapper))
                valid_mask_2d.append([True,False,True])
                if i+1 >len(self.annots[imgpath]['smpl_params']):
                    valid_mask_3d.append([True,False,False,False,False,True])
                else:
                    valid_mask_3d.append([True,True,True,True,False,True])
                    # valid_mask_3d.append([True,False,False,False,False,True])
                # valid_mask_3d.append([False,False,False,False,False,True])
            ...
            for smpl_param in smpl_params:
                global_orient = np.array(smpl_param['Rh'])
                poses = np.array(smpl_param['poses'])
                betas = np.array(smpl_param['shapes'])
                poses[0, :3] = global_orient
                param = np.concatenate((poses, betas), axis=-1)
                params.append(param)
            params=np.array(params).squeeze(1)
            ...
            img_info = {'imgpath': imgpath, 'image': image, 'kp2ds': kp2ds, 'track_ids': np.arange(len(kp2ds)),
                        ...
                        'camMats': camMats, 'img_size': image.shape[:2], 'ds': 'scut_mp'}


            return img_info

        def pack_data(self, annots_file_path):
            print('SCUT_3DHP_MP data annotations is packing...')
            self.annots = {}
            # frame_info = {}
            ...
            for i, video_name in enumerate(video_names):
                ...
                cam_dir = self.image_folder.replace('RGB_frame', 'Cam')
                for frame_name in frame_names:
                    ...
                    kp3d = read_3d_keypoints(label_3d_path)
                    smpl_param = read_smpl_param(label_smpl_path)
                    # read cam params
                    cam_name = frame_name.split('.')[0][:4]
                    cam_params = read_camera(os.path.join(cam_dir, 'intri.yml'),
                                             os.path.join(cam_dir, 'extri.yml'))
            ...
            print('SCUT_3DHP_MP data annotations packed')
...
def read_2d_keypoints(anno_path):
    ...
    for person_id in range(len(all_skeleton_sequence)):
        keypoints = []
        if person_id not in all_skeleton_sequence:
            print(anno_path)
            for i in range(21):
                keypoint=[-1, -1]
...
def read_3d_keypoints(anno_path):
    ...
    all_keypoints=[]
    for person_id in sorted(all_skeleton_sequence.keys()):
        keypoints = []
        skeleton_sequence = all_skeleton_sequence[person_id]
        for label in range(21):
            if skeleton_sequence.get(f'{label:02d}') == None:
...
def read_smpl_param(anno_path):
    ...
    smpl_params=sorted(smpl_params, key=lambda x: x['id'])
    return smpl_params
...
if __name__ == '__main__':
    dataset = SCUT_3DHP_MP(base_class=default_mode)(train_flag=True, regress_smpl=True)
